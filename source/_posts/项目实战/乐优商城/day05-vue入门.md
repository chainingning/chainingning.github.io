...
使用`花括号`方式在网速较慢时会出现问题。在数据未加载完成时，页面会显示出原始的`花括号`，加载完毕后才显示正确数据，我们称为插值闪烁。
...
解决上面的问题，可以使用v-text和v-html指令来替代`花括号`
...
html属性不能直接使用双大括号形式绑定，只能使用v-bind指令。
...
Vue中提供了计算属性（这里注意属性和方法，属性是没有()的，方法才有），来替代复杂的表达式：
